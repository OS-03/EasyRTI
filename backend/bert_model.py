...
def get_classifier_model():
    if not hasattr(get_classifier_model, "model"):
        classifier_model_name = "conan04/bert_rti_multilingual"
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        get_classifier_model.model = AutoModelForSequenceClassification.from_pretrained(classifier_model_name).to(device)
    return get_classifier_model.model
